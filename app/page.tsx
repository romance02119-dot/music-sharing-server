...
import { 
  ...
  MessageSquare, CornerDownRight, Plus, Minus, LayoutGrid, Folder, Edit2, ArrowUp, Lock, SearchX, Music2 
} from 'lucide-react';
import { User } from "@supabase/supabase-js";

// --- 1. 타입 정의 (기존 이름 유지) ---
interface Post { id: number; title: string; artist: string; description: string; youtube_id?: string; likes: number; views?: number; mood: string; genre: string; }
...
interface Comment { id: string; post_id: number; content: string; created_at: string; parent_id: string | null; user_id?: string; likes_count?: number; updated_at?: string; profiles?: { name?: string; avatar_url?: string; } | null; }
...
function CommentSection({ postId, user, currentPost, onSeekToTime }: { 
  ...
  const fetchComments = async () => {
    const { data } = await supabase.from('music_comments').select('*, profiles!user_id(name, avatar_url), likes_count').eq('post_id', postId).order('created_at', { ascending: true });
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      supabase.from('profiles').upsert({ id: user.id, name: user.user_metadata?.name || '익명', avatar_url: avatar }, { onConflict: 'id' }).then();
    }
  }, [postId, user]);
  ...
  const deleteComment = async (id: string) => {
    if (confirm('삭제하시겠습니까?')) {
      await supabase.from('music_comments').delete().eq('id', id);
      fetchComments();
    }
  };
  ...
  const handleLikeComment = async (id: string) => {
    ...
    setComments(prev => prev.map(c => c.id === id ? { ...c, likes_count: newLikes } : c));
    await supabase.from('music_comments').update({ likes_count: newLikes }).eq('id', id);
  };

  const handleSaveEdit = async (id: string) => {
    if (!user || !editContent.trim()) return;
    const { error } = await supabase.from('music_comments').update({ content: editContent.trim(), updated_at: new Date().toISOString() }).eq('id', id).eq('user_id', user.id);
  ...
  const parseTimestamps = (content: string) => {
    ...
    return parts.map((part, i) => part.match(/^\d{1,2}:\d{2}$/) ? (
      <button key={i} onClick={() => onSeekToTime?.(parseInt(part.split(':')[0]) * 60 + parseInt(part.split(':')[1]))} className="text-purple-400 font-black text-xs underline mx-1 hover:text-purple-300">
        {part}
      </button>
    ) : part);
  };

  const RenderComment = ({ c, isReply = false }: { c: Comment, isReply?: boolean }) => (
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-semibold text-purple-400 cursor-pointer" onClick={(e) => {
                const rect = (e.target as HTMLElement).getBoundingClientRect();
                setProfilePopupPosition({ x: rect.left + window.scrollX + rect.width / 2, y: rect.top + window.scrollY - 10 });
                supabase.from('profiles').select('id, name, avatar_url, music_comments(count)').eq('id', c.user_id).single().then(({data}) => data && setSelectedProfile({id: data.id, name: data.name, avatar_url: data.avatar_url, comment_count: (data.music_comments as any).length}));
              }}>{c.profiles?.name || '익명'}</span>
            ...
            {editingComment === c.id ? (
              <div className="space-y-2">
                <textarea value={editContent} onChange={e => setEditContent(e.target.value)} className="w-full p-2 bg-black border border-purple-500/30 rounded text-sm text-slate-200" rows={2} autoFocus />
                <div className="flex gap-2">
                  <button onClick={() => handleSaveEdit(c.id)} className="px-3 py-1 bg-purple-600 text-white text-[10px] font-black rounded">저장</button>
            ...
            <div className="mt-2 flex items-center gap-4">
              {!isReply && <button onClick={() => setReplyTo(c)} className="text-[8px] text-purple-400 font-black">답글 달기</button>}
              <button onClick={() => handleLikeComment(c.id)} className="flex items-center gap-1 text-[8px] text-pink-400 font-black">
                <Heart size={10} className={c.likes_count ? "fill-current text-pink-500" : ""} /> {c.likes_count || 0}
              </button>
              {user?.id === c.user_id && (
                <div className={`flex gap-2 ml-auto transition-opacity ${isReply ? 'opacity-0 group-hover/reply:opacity-100' : 'opacity-0 group-hover/comment:opacity-100'}`}>
                  <button onClick={() => deleteComment(c.id)} className="text-[8px] text-red-400 font-black">삭제</button>
                  <button onClick={() => { setEditingComment(c.id); setEditContent(c.content); }} className="text-[8px] text-blue-400 font-black">수정</button>
                </div>
              )}
            </div>
          </div>
    ...
    <div className="mt-6 pt-4 border-t border-slate-800/30" onClick={() => setSelectedProfile(null)}>
      <div className="space-y-4 mb-4 max-h-60 overflow-y-auto no-scrollbar px-1">
        {comments.filter(c => !c.parent_id).map(c => (
          <div key={c.id}>
            <RenderComment c={c} />
            {comments.filter(r => r.parent_id === c.id).map(reply => <RenderComment key={reply.id} c={reply} isReply />)}
          </div>
        ))}
      </div>
      ...
      {selectedProfile && profilePopupPosition && (
        <div 
          className="fixed z-[9999] bg-[#1a1a1e] border border-purple-500/30 rounded-lg shadow-xl p-4 min-w-[200px] animate-in fade-in slide-in-from-top-2 duration-200"
          style={{ 
            left: `${profilePopupPosition.x}px`, 
            top: `${profilePopupPosition.y}px`,
          ...
          <div className="flex items-center gap-3 mb-3">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center flex-shrink-0">
              {selectedProfile.avatar_url ? (
                <img src={selectedProfile.avatar_url} alt="profile" className="w-full h-full rounded-full object-cover" />
              ) : (
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-200">{selectedProfile.name}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-2">
          {replyTo && (
            <div className="flex items-center justify-between bg-purple-500/10 p-2 rounded-lg border border-purple-500/20 animate-pulse">
              <span className="text-[9px] text-purple-600 font-bold flex items-center gap-1">
                <MessageSquare size={10} /> "{replyTo.content.substring(0, 10)}..." 님께 답글 작성 중
              </span>
    ...
    <span>
      {parts.map((part, i) => 
        part.toLowerCase() === query.toLowerCase() ? (
          <mark key={i} className="bg-purple-500/30 text-purple-400 bg-transparent p-0">
            {part}
          </mark>
        ) : (
          part
...
export default function MichidaUltimateFinalFix() {
  ...
  useEffect(() => {
    fetchPosts();
  }, [selectedPlaylist, selectedFolderId]);
  ...
  const fetchPosts = async () => {
    let query = supabase
      .from('music_posts')
      .select('id, title, artist, description, youtube_id, likes, views, mood, genre, created_at')
    ...
    if (selectedFolderId) {
      ...
      if (folderItems && folderItems.length > 0) {
        const postIds = folderItems.map(item => item.post_id);
  ...
  const fetchMyPlaylist = async () => {
    ...
    const { data, error } = await supabase
      .from('user_playlists')
      .select(`id, folder_id, music_posts (id, title, artist)`)
      .eq('user_id', user.id)
      .order('created_at', { ascending: false });
    if (!error && data) {
...
const handleUpdateDesc = async (id: number, content: string) => {
    ...
    <div className="min-h-screen bg-[#0a0a0c] text-slate-200 p-4 md:p-8">
      <div className="max-w-[1400px] mx-auto">
        <header className="flex items-center justify-between mb-12">
          ...
          >
            <div className="bg-gradient-to-tr from-purple-600 to-blue-500 p-2 rounded-xl shadow-lg shadow-purple-500/20">
              <Music className="text-white" size={28} />
            </div>
            <h1 className="text-3xl font-black italic tracking-tighter bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent">
              MICHIDA
            </h1>
          </div>
          ...
          {user ? (
            ...
            <button
              onClick={handleLogin}
              className="px-4 py-1.5 rounded-lg bg-white text-black text-xs font-black hover:bg-purple-500 hover:text-white transition-all"
            >
              GOOGLE LOGIN
            </button>
          )}
  ...
  <h2 className="text-sm font-black mb-6 text-purple-400 flex items-center gap-2 uppercase tracking-tighter">
    <Upload size={18} /> 곡 공유하기
  </h2>
  ...
  {user ? (
    <form onSubmit={handleUpload} className="space-y-3">
      <input 
        ...
        placeholder="제목" 
        className="w-full bg-[#0a0a0c] rounded-xl p-3 text-xs border border-slate-800 outline-none focus:border-purple-500/50" 
        required
      />
      ...
      <textarea 
        ...
        placeholder="곡에 대한 짤막한 소개를 남겨주세요 (예: 비 오는 날 듣기 좋아요)" 
        className="w-full bg-[#0a0a0c] rounded-xl p-3 text-xs border border-slate-800 outline-none focus:border-purple-500/50 min-h-[80px] resize-none"
      />
      ...
      <button type="submit" className="w-full bg-gradient-to-r from-purple-600 to-blue-600 py-3 rounded-xl font-black text-xs hover:opacity-90 transition-opacity">
        공유하기
      </button>
    </form>
  ) : (
    <div className="bg-[#16161a] p-10 rounded-3xl border border-dashed border-white/10 text-center">
      ...
      <button 
        onClick={handleLogin}
        className="w-full h-12 flex items-center justify-center bg-purple-600 rounded-full text-white font-bold transition-transform hover:scale-105 active:scale-95 hover:bg-purple-700"
      >
        <span className="leading-none">1초만에 로그인하기</span>
      </button>
    </div>
  )}
</div>
  ...
  <div className="bg-[#141418] p-6 rounded-[2rem] border border-blue-500/10 shadow-xl">
    <div className="flex justify-between items-center mb-6">
      <h2 className="text-[11px] font-black text-blue-400 flex items-center gap-2 uppercase tracking-[0.2em]">
        <HeadphoneOff size={14} /> 최근 재생한 노래
      </h2>
      <button
        onClick={clearAllRecents}
        className="text-[9px] text-slate-600 hover:text-red-400 font-bold underline"
      >
        전체 삭제
      </button>
    </div>
    ...
    <div className="space-y-2">
      {recentlyViewed.map(post => (
        <div key={post.id} className="flex items-center group gap-2 min-w-0">
          <div
            onClick={() => handlePlayAndScroll(post)}
            className={`flex-1 min-w-0 flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${
              currentPlayingId === post.id
                ? "bg-purple-600/20 border border-purple-500/30"
                : "bg-[#0a0a0c] border border-transparent hover:border-slate-800"
            }`}
          >
            {currentPlayingId === post.id
              ? <div className="w-1 h-3 bg-purple-500 animate-bounce flex-shrink-0" />
              : <div className="w-1 h-3 bg-slate-800 flex-shrink-0" />
            }
            <div className="flex flex-col min-w-0 flex-1">
              <span
                className={`text-sm font-bold truncate block ${
                  currentPlayingId === post.id ? "text-purple-400" : "text-white"
                }`}
              >
                {post.title}
              </span>
          ...
          <button
            onClick={() => removeRecent(post.id)}
            className="opacity-0 group-hover:opacity-100 p-2 text-slate-600 hover:text-red-500 transition-all flex-shrink-0"
          >
            <Trash2 size={14} />
          </button>
        </div>
          ...
          <main className="lg:col-span-6 space-y-8">
            {/* 검색바 컨테이너 */}
            <div className={`sticky top-0 z-40 py-4 bg-[#0a0a0c]/80 backdrop-blur-md transition-all ${
              showTopButton ? "border-b border-white/10 shadow-2xl" : ""
            }`}>
              {/* 폴더 필터 인디케이터 */}
              {selectedFolderId && (
                <div className="mb-3 p-3 bg-purple-600/10 border border-purple-500/20 rounded-xl flex items-center justify-between animate-in slide-in-from-top-2 duration-300">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-purple-300 font-medium">
                      {playlistFolders.find(f => f.id === selectedFolderId)?.name} 폴더의 곡들을 보고 있습니다
                    </span>
                  </div>
                  <button 
                    onClick={handleClearFolderFilter}
                    className="text-xs text-purple-400 hover:text-purple-300 font-black underline underline-offset-2 transition-colors"
                  >
                    필터 해제
                  </button>
                </div>
              ...
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full bg-[#16161a] p-4 pl-12 rounded-2xl border border-white/5 focus:ring-2 focus:ring-purple-500 outline-none transition-all shadow-xl" 
                  placeholder="당신의 감성을 검색하세요" 
                />
              </div>
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
              {moods.map(m => (
                <button key={m} onClick={() => setSelectedMood(m)} className={`px-4 py-2 rounded-full text-[10px] font-black whitespace-nowrap transition-all ${selectedMood === m ? "bg-purple-600 text-white shadow-lg shadow-purple-500/20" : "bg-[#141418] text-slate-500 border border-slate-800 hover:border-slate-700"}`}>{m}</button>
              ))}
            </div>
                ...
                <div className="flex flex-col justify-center items-center py-32 text-center">
                  {selectedFolderId ? (
                    <>
                      ...
                      <h3 className="text-2xl font-light text-slate-300 mb-3">이 폴더는 아직 비어있어요. 🎵</h3>
                      <p className="text-slate-500 mb-8">좋아하는 곡을 담아서 나만의 리스트를 만들어보세요!</p>
                      <button 
                        onClick={handleClearFolderFilter}
                        className="px-8 py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-2xl text-sm font-black hover:from-purple-700 hover:to-blue-700 transition-all duration-300 shadow-lg shadow-purple-500/20 hover:shadow-xl hover:shadow-purple-500/30 transform hover:scale-105"
                      >
                        전체 곡 보러가기
                      </button>
                    </>
                    ...
                    <>
                      <SearchX size={48} className="mb-4 text-slate-600 opacity-50" />
                      <p className="text-lg text-slate-400">앗, 찾는 곡이 아직 없네요!</p>
                      <p className="text-sm text-slate-500">다른 키워드로 검색하거나 직접 공유해보세요.</p>
                    </>
                  )}
                </div>
              )}
              {filteredPosts.map((post) => (
                <div key={post.id} className="animate-in fade-in slide-in-from-bottom-3 duration-500 ease-out">
                  <article 
                      ...
                      {activeYoutube.includes(post.id) ? (
                        ...
                        <div className="absolute inset-0 bg-[#0a0a0c]/60 flex items-center justify-center cursor-pointer" onMouseDown={() => handlePlayAndScroll(post)}>
                          <div className="relative flex items-center justify-center w-16 h-16 bg-[#8b3dff] rounded-full shadow-[0_0_20px_rgba(139,61,255,0.4)] hover:scale-110 transition-transform">
                            <Play 
                              size={32} 
                              className="text-white fill-current transform translate-x-[2px]" 
                            />
                          </div>
                    ...
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <div className="flex gap-2">
                        ...
                        {user && selectedPlaylist && (
                          <button 
                            onClick={() => handleRemoveFromPlaylist(post.id)}
                        ...
                        <button onClick={() => handleDeletePost(post.id)} className="p-3 rounded-2xl bg-[#0a0a0c] text-slate-700 hover:text-red-500 border border-slate-800 transition-all"><Trash2 size={18} /></button>
                      </div>
                    </div>
<div className="mb-6">
  {editingId === post.id ? (
    <div className="space-y-2 animate-in fade-in duration-300">
      <textarea
        ...
        className="w-full bg-[#0a0a0c] rounded-xl p-3 text-sm text-slate-300 border border-purple-500/50 outline-none min-h-[80px] resize-none"
        placeholder="곡 소개를 입력하세요..."
      />
      <div className="flex justify-between items-center">
        {/* 왼쪽: 삭제 버튼 (빨간색 계열 포인트) */}
        <button 
          onClick={() => {
            if(confirm('소개글을 삭제하시겠습니까?')) handleUpdateDesc(post.id, '');
          }} 
          className="text-[10px] font-black text-red-500/70 hover:text-red-500 transition-colors"
        >
          삭제하기
        </button>
        ...
        <div className="flex gap-3">
          ...
          <button 
            onClick={() => handleUpdateDesc(post.id, editDesc)} 
            className="text-[10px] font-black text-purple-400 hover:text-purple-300 transition-colors"
          >
            저장하기
          </button>
        </div>
    ...
    <div className="group/desc relative">
      <p className="text-slate-400 text-sm italic border-l-4 border-purple-500/20 pl-4 py-1 leading-relaxed">
        {post.description ? `"${post.description}"` : '"소개글이 없습니다. 내용을 추가해보세요!"'}
      </p>
      <button 
        onClick={() => {
        ...
        className="absolute top-0 right-0 opacity-0 group-hover/desc:opacity-100 text-[9px] font-black text-slate-600 hover:text-purple-400 transition-all"
      >
        수정
      </button>
    </div>
                    ...
                    <div className="flex items-center gap-4 mt-4 text-xs text-slate-500">
                      <div className="flex items-center gap-1">
                        <Play size={12} /> {post.views || 0} VIEWS
                      </div>
                    ...
                    <CommentSection 
                      ...
                      onSeekToTime={(seconds) => {
                        // YouTube 플레이어 Seek 기능 구현
                        if (typeof window !== 'undefined') {
                          console.log('Seeking to:', seconds, 'seconds');
                          
                          // 모든 YouTube iframe 찾기
                          const iframes = document.querySelectorAll('iframe[src*="youtube"]');
                          
                          iframes.forEach((iframe) => {
                            const iframeEl = iframe as HTMLIFrameElement;
                            console.log('Found iframe:', iframeEl.src);
                            ...
                            if (iframeEl.contentWindow) {
                              ...
                              try {
                                ...
                                console.log('Seek command sent to iframe');
                              } catch (error) {
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-[#141418]/50 p-4 rounded-[2.5rem] border border-slate-800/50 flex flex-col gap-1 backdrop-blur-sm">
                  {genres.map(g => (
                    <button key={g} onClick={() => handleGenreSelect(g)} className={`flex items-center justify-between p-3 rounded-xl text-[11px] font-bold transition-all ${selectedGenre === g ? "bg-blue-600 text-white shadow-lg shadow-blue-500/20" : "bg-transparent text-slate-600 hover:bg-slate-800/50 hover:text-slate-300"}`}>
              ...
              <div className="bg-[#16161a] p-6 rounded-3xl border border-white/5 shadow-xl">
                ...
                <div className="space-y-4">
                  {topPosts.map((post, idx) => (
                    <div key={post.id} onClick={() => handlePlayAndScroll(post)} className="flex items-center gap-4 group cursor-pointer transition-transform hover:translate-x-1">
              ...
              {/* 3. MY PLAYLIST */}
              <div className="bg-[#16161a] p-6 rounded-3xl border border-white/5 shadow-xl transition-all hover:border-purple-500/30">
                ...
                <div className="space-y-4">
                  {playlistFolders.length === 0 ? (
                    <p className="text-sm text-slate-500 py-4 text-center">아직 폴더가 없어요!</p>
                  ) : (
                    playlistFolders.map(folder => (
                      <div key={folder.id} className="space-y-2">
                        <div 
                          className={`flex items-center justify-between p-2 rounded-xl cursor-pointer transition-all ${
                            selectedFolderId === folder.id 
                              ...
                              : "bg-purple-900/10 border border-purple-500/20 hover:bg-purple-800/20"
                          }`}
                        >
                          <div 
                          ...
                          >
                            <Folder 
                              size={16} 
                              className={`${selectedFolderId === folder.id ? "text-purple-300" : "text-purple-400"}`} 
                            />
                            <span className={`text-sm font-medium ${
                              selectedFolderId === folder.id ? "text-purple-200" : "text-purple-300"
                            }`}>{folder.name}</span>
                            {selectedFolderId === folder.id && (
                              <span className="text-[9px] bg-purple-500 text-white px-2 py-0.5 rounded-full font-black animate-pulse">선택됨</span>
                            )}
                          </div>
                          <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                            <button 
                              onClick={() => handleUpdateFolderName(folder.id, folder.name)}
                        ...
                        <div className="ml-4 space-y-1">
                          {myPlaylist
                            .filter(item => item.folder_id === folder.id && item.music_posts)
                            .slice(0, 3)
                            .map(item => {
                              const post = item.music_posts;
                              if (!post) return null;
                              const fullPost: Post = {
                                id: post.id,
                                title: post.title,
                              ...
                              return (
                                <div 
                                  key={item.id} 
                                  onClick={() => handlePlayAndScroll(fullPost)} 
                                  className="flex items-center gap-2 p-2 rounded-lg hover:bg-white/5 transition-colors cursor-pointer group"
                                >
                                  <div className="w-8 h-8 bg-purple-900/20 rounded flex items-center justify-center">
                                    <Play size={12} className="text-purple-500 opacity-0 group-hover:opacity-100 transition-opacity" />
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <p className="text-xs font-medium truncate text-slate-300">{post.title}</p>
                                    <p className="text-[9px] text-slate-500">{post.artist}</p>
                                  </div>
                                </div>
                              );
                            })}
                          {myPlaylist.filter(item => item.folder_id === folder.id && item.music_posts).length === 0 && (
                            <p className="text-[9px] text-slate-600 text-center py-2">이 폴더에 담긴 곡이 없어요</p>
                          )}
                        </div>
                      </div>
                    ))
      ...
      {isSelectModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-[#141418] border border-white/10 w-full max-w-xs rounded-[2rem] p-6">
            <h3 className="text-white font-black mb-4 flex items-center justify-between">
              폴더 선택 <button onClick={() => setIsSelectModalOpen(false)}><X size={20}/></button>
            </h3>
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {playlistFolders.length === 0 ? (
                <p className="text-slate-400 text-sm text-center py-4">폴더가 없습니다. 먼저 폴더를 생성해주세요!</p>
              ) : (
                playlistFolders.map(folder => (
                  <button 
                    key={folder.id}
                    onClick={() => saveToPlaylist(folder.id)}
                    className="w-full p-4 bg-white/5 hover:bg-purple-600 rounded-xl text-left text-xs font-bold transition-all text-white flex items-center gap-2"
                  >
                    <Folder size={14} />
                    {folder.name}
                  </button>
      ...
      {showTopButton && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-10 right-10 p-4 bg-purple-600 hover:bg-purple-500 text-white rounded-full shadow-lg transition-all duration-300 z-50 animate-bounce-subtle"
          aria-label="맨 위로 이동"
        >
          <ArrowUp size={24} />
        </button>
